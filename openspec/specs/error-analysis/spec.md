...
Explain detected errors using a local LLM, producing a structured verdict with
a root cause, severity, and confidence score.

Analysis runs asynchronously so that inference latency never stalls log reading,
and is grounded in operator-supplied documentation so that explanations reflect
the specific deployment rather than generic assumptions.
