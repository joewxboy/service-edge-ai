...
Forward detected errors and remediation proposals to systems outside the node —
observability platforms, single-pane-of-glass control centres, and issue
trackers — so that problems are visible across a fleet rather than only on the
node that produced them.

Export is optional and inert unless configured. It deliberately and selectively
relaxes the guarantee that log content never leaves the node, so what each sink
transmits is governed by an explicit redaction level, gated on the monitored
workload's own consent, and recorded on the proposal. No export failure may
affect error detection, analysis, or local proposal storage.
