...
Tail the log files of workloads that have opted into monitoring, detect error
lines, and capture enough surrounding context for an analysis to be meaningful.

Monitoring is permission-based and read-only. Resource use is bounded so that
watching a workload never degrades it.
