...
Discover which Open Horizon workloads exist on the local node, track their
lifecycle state, and determine which of them have consented to being monitored.

Discovery is read-only: the system queries the local anax agent and never
mutates node state. Consent is expressed by the workload itself through
`MONITORING_*` variables in its deployment string, so a workload that has not
opted in can never be monitored.
